...
class ReminderManager(models.Manager):
    ...
    def create_reminder(self,name,date,user,event_associated_flag=False,associated_event=None,picture=None,type=None,reminder_for=None):
        reminder = self.create(name=name,reminder_date=date,user=user,event_associated_flag=event_associated_flag,associated_event=associated_event,picture=picture,type=type,reminder_for=reminder_for,creation_date = datetime.datetime.now())
        return reminder
...
class NotificationManager(models.Manager):
    def create_notification(self,message,associated_user,url,is_read=False,read_date=None,type=None,event_reminder_type=None,event_for_user=None):
        notification = self.create(message=message,associated_user=associated_user,creation_date=datetime.datetime.now(),url=url,is_read=False,type=type,event_reminder_type=event_reminder_type,event_for_user=event_for_user)
        return notification
...
class ChatManager(models.Manager):
    def create_event_chat(self, event, user,wish = None,message_field=None, url_field=None, file_field = None, type='LR', creation_date = datetime.datetime.now()):
        if message_field is None and url_field is None and file_field is None:
            ...
            event = self.create(event=event,wish = wish,message_field = message_field, url_field = url_field, file_field = file_field, user=user, type=type,creation_date = creation_date)
            return event
...
class FileTestMode(models.Model):
    file_field = models.ImageField(upload_to='images/%Y/%m/%d',null=True)
